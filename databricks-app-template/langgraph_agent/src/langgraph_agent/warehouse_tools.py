...
# =========================
FULL_TABLE_NAME = f"`{UC_CATALOG}`.`{UC_SCHEMA}`.`nutr_alias_lookup`"
...
def _get_similarity_llm() -> ChatDatabricks:
    ...
    if not config.model_endpoint:
        raise ValueError("DATABRICKS_MODEL_ENDPOINT is required for column matching.")
    ...
    for query_entry in normalized_queries:
        ...
        for match in best_matches:
            match_payload.append(
                {
                    "original_column_name": match["original_column_name"],
                    "column_meaning": match["column_meaning"],
                    "original_table_name": match["original_table_name"],
...
def _qualify_table_name(
    table_name: str, schema_name: str = "data_for_bi_dashboards_dev"
) -> str:
    if "." in table_name:
...
match_nutrient_names_tool = tool(match_nutrient_names)
match_column_names_tool = tool(match_column_names)
query_nutrient_data_tool = tool(query_nutrient_data)
